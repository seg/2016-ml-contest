...
#On Prod dataset
rpart.prod.pred = predict(rpart.local.model, type='class',newdata = test_prod)
...
ensemble_DF = data.frame(XGB = (xgb.prod.pred),
                         ...
                         rpart   = as.numeric(rpart.prod.pred),
                         gbm_h2o = as.numeric(gbm.prod.pred.h2o$predict)
                         )
...
write.csv(sub, row.names= F, 'Ensemble_2.csv')
