...
well_names = c("CHURCHMAN BIBLE", "SHRIMPLIN", "CROSS H CATTLE", "NOLAN", "SHANKLE", "NEWBY"
               ,"Recruit F9", "LUKE G U")
RF.local.pred1 = NULL
RF.prod.pred1 = NULL
for (i in well_names[!well_names %in% "SHRIMPLIN"])
{
  train_local = train_prod[!train_prod$Well.Name %in% c(i,'SHRIMPLIN'),]
  test_local  = train_prod[train_prod$Well.Name %in% 'SHRIMPLIN',]
  set.seed(100)
  ...
  RF.local.pred1[[i]] = RF.local.pred
  acc_table_RF = table(RF.local.pred, test_local$Facies)
  acc_RF = sum(diag(acc_table_RF))/nrow(test_local)
  acc_RF
  F1(acc_table_RF)
  print(paste(i, 'Acc', acc_RF))
  print(paste(i, 'F1', F1(acc_table_RF)))
  #On Prod
  RF.prod.pred  = predict(RF.local.model, newdata = test_prod)
  RF.prod.pred1[[i]] = RF.prod.pred
  
}

#Converting test local and test prod to dataframe 
RF.local.pred1 = as.data.frame(RF.local.pred1)
RF.prod.pred1 = as.data.frame(RF.prod.pred1)

##########
#Ensembling the test local
ensemble_DF = RF.local.pred1
#Finding the mode
getmode <- function(v) {
  uniqv <- unique(v)
...
ensemble_DF$predicted = predicted
ensemble_DF$predicted = factor(as.character(ensemble_DF$predicted), levels = levels(test_local$Facies))
...
##########
#Ensembling the test prod
ensemble_DF = RF.prod.pred1
#Finding the mode
getmode <- function(v) {
  uniqv <- unique(v)
  uniqv[which.max(tabulate(match(v, uniqv)))]
...
sub = cbind(test_prod, Facies = predicted)
write.csv(sub, row.names= F, 'RF_predicted_facies_ensemble1_6.csv')
