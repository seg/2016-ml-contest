...
train_prod_dum = dummy.data.frame(train_prod[!colnames(train_prod) %in% c('Well.Name','Facies')], sep='_')
train_prod = cbind(as.data.frame(train_prod_dum), Facies = train_prod[,'Facies'])
str(train_prod)
...
train_xgb_indep = train_prod[,!colnames(train_prod) %in% c(#'Formation', 
                                                             'Well.Name',
                                                             #'RELPOS_next',
                                                             'RELPOS_previous',
                                                             'ILD_log10_next',
                                                             ...
                                                             'Phi_diff',
                                                             'Facies')]
train_xgb_label = train_prod[,'Facies']
...
{ 
  ...
  labels <- factor(labels, levels = levels(train_prod$Facies))
  acc_table = table(preds, labels)
  Acc = F1(acc_table)
  return(list(metric = "F1", value = Acc))
}

dtrain_prod = xgb.DMatrix(data = as.matrix(train_xgb_indep), label = as.matrix(train_xgb_label))
...
write.csv(sub, row.names= F, 'XGB_predicted_facies_13.csv')
