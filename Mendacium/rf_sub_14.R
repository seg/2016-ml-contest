...
train_prod$Formation = as.factor(train_prod$Formation)
test_prod$Formation  = as.factor(test_prod$Formation)
...
set.seed(100)
rf_model <- train(Facies~., method = "parRF", trControl = control, 
                  prox = TRUE, allowParallel = TRUE,
                  data = train_local[,!colnames(train_local) %in% c(
                    #'Formation',
                    #'Depth',
                    #'RELPOS_next',
                    'RELPOS_previous',
...
set.seed(100)
RF.local.model = randomForest(Facies~., mtry =12, 
                              data = train_local[,!colnames(train_local) %in% c(
                                #'Formation',
                                #'Depth',
                                #'RELPOS_next',
...
write.csv(sub, row.names= F, 'RF_predicted_facies_15.csv')
