firstDiff <- function(data, diffPE=T) {
    ...
    for (well_i in wells) {
        ...
        for (j in 1:nrow(data_i)) {
            ...
            if (j == 1) {
                ...
                if (c("PE") %in% names(data_i) & diffPE) {
...
lagData <- function(data, l, features=c("GR", "ILD_log10", "DeltaPHI", "PHIND", "isMarine")) {
    ...
    for (well_i in wells) {
        ...
        for (i in (-l/2):(l/2)) {
            suffix <- ifelse(i < 0, paste0("n", abs(i)), i)
        
            for (f in features) {
                if (i != 0) {
                    data_i[, paste0(f, "_", suffix)] <- lagpad(data_i[[f]], i)    
                }
            }
        }
